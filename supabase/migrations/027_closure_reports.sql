-- Player-reported closures: "this court is closed / under maintenance", shown
-- on the court card as a soft, attributed banner — never as a schedule change.
-- Folded into schema/11_closure_reports.sql. Idempotent: safe to re-run, which is
-- how an existing 027 picks up the email trigger added below. The email half
-- reuses 028's Vault secrets and does nothing without them.
--
...
-- A report's note is user-written text shown to everyone, so it is reportable
-- like reviews and chat (App Store UGC rule): content_reports.kind 'closure',
-- ref_id = the closure report id.
...
alter table public.content_reports
  ...
  check (kind in ('message', 'review', 'signal', 'profile', 'run', 'data', 'issue', 'closure'));

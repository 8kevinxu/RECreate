// "Down to play" signals: location-less availability pings to friends.
...
export async function loadSignals() {
  ...
  const { data, error } = await supabase
    .from('rec_signals')
    .select(
      'id, user_id, starts_at, sport, note, place, pref_court_id, planned_at, planned_court_id, created_at,' +
        ' user_profile:profiles!user_id(display_name),' +
        ' rec_signal_participants(user_id, proposed_at, proposed_court_id, proposed_sport, profiles!user_id(display_name))'
    )
    .gt('expires_at', nowIso)
    .order('starts_at', { ascending: true, nullsFirst: true });
  if (error || !data) return [];
  // Hide blocked users' signals entirely, and drop them from participant lists.
  const blocked = await getBlockedIds();
  return data
    .filter((s) => !blocked.has(s.user_id))
    .map((s) => {
    const participants = (s.rec_signal_participants || [])
      ...
      proposedSport: p.proposed_sport,
    }));
    ...
    return {
      ...
      isNow: !s.starts_at,
      sport: s.sport || ANY_SPORT,
      note: s.note,
      place: s.place || null, // 'indoor' | 'outdoor' | null (either)
      prefCourtId: s.pref_court_id || null, // creator's preferred court | null
      ...
      myProposedSport: me_part?.proposedSport ?? null,
    };
  });
}

// Post a signal. startsAt is a Date (scheduled) or null ("right now"). sport may be
// ANY_SPORT ("just down for rec") when the poster doesn't pick a specific sport.
// place ('indoor'|'outdoor') and prefCourtId are optional up-front preferences —
// null means "let friends decide".
export async function createSignal({
  ...
  const me = await currentUserId();
  if (!me) return { error: new Error(tg('err.signInPost')) };
  const start = startsAt ? new Date(startsAt) : null;
  const base = start ? start.getTime() : Date.now();
  const { error } = await supabase.from('rec_signals').insert({
    user_id: me,
    starts_at: start ? start.toISOString() : null,
    sport,
    note: note?.trim() || null,
    place: place || null,
    pref_court_id: prefCourtId || null,
    notify,
...
export async function cancelSignal(id) {
  if (!supabase) return { error: new Error(tg('err.notConfigured')) };
  const { error } = await supabase.from('rec_signals').delete().eq('id', id);
  return { error };
}
...
export async function joinSignal(
  ...
  if (!supabase) return { error: new Error(tg('err.notConfigured')) };
  const me = await currentUserId();
  if (!me) return { error: new Error(tg('err.signInJoin')) };
  const { error } = await supabase.from('rec_signal_participants').upsert(
    {
      signal_id: signalId,
      user_id: me,
      proposed_at: proposedAt ? new Date(proposedAt).toISOString() : null,
      proposed_court_id: proposedCourtId || null,
      proposed_sport: proposedSport || null,
    },
...
export async function leaveSignal(signalId) {
  if (!supabase) return { error: new Error(tg('err.notConfigured')) };
  const me = await currentUserId();
  if (!me) return { error: new Error(tg('err.notSignedIn')) };
  const { error } = await supabase
    .from('rec_signal_participants')
    .delete()
...
export async function confirmSignalTime(
  signalId,
  plannedAt,
  plannedCourtId = null,
  plannedSport = null
  ...
  if (plannedAt) {
    ...
    // Promote the agreed sport/activity onto the signal (e.g. an "Anything" signal
    // becomes concrete once the host confirms a basketball suggestion).
    if (plannedSport) patch.sport = plannedSport;
  }
  const { error } = await supabase
    .from('rec_signals')
    .update(patch)
...
export function subscribeSignals(onChange) {
  ...
  try {
    const channel = supabase
      .channel(`rec_signals_${++channelSeq}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'rec_signals' },
        onChange
      )
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'rec_signal_participants' },
        onChange
      )
      .subscribe();
    return () => {

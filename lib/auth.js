// Account state for RECreate, backed by Supabase Auth (email + password).
...
const DISABLED = {
  ...
  resetPassword: async () => ({ error: new Error(tg('err.notConfigured')) }),
  verifyResetCode: async () => ({ error: new Error(tg('err.notConfigured')) }),
  updatePassword: async () => ({ error: new Error(tg('err.notConfigured')) }),
  signOut: async () => {},
...
const PROFILE_COLS =
  'id, display_name, age, bio, neighborhood, favorite_sports, favorite_categories, share_activity';
...
export function AuthProvider({ children }) {
  ...
  // Forgot-password flow, code-based so no deep link is needed: email a 6-digit
  // recovery code (the Supabase "Reset Password" email template must include
  // {{ .Token }}), verify it — which starts a recovery session — then set the
  ...
  const updateProfile = async (fields) => {
    ...
    if ('favorite_categories' in fields) {
      patch.favorite_categories = fields.favorite_categories?.length
        ? fields.favorite_categories
        : null;
    }
    if ('share_activity' in fields) patch.share_activity = !!fields.share_activity;

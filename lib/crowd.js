...
const STORE_KEY = 'recreate.crowd.v2'; // local history array per court
const MY_KEY = 'recreate.myvotes.v1'; // this device's own vote per court (for toggle/undo)
...
async function supaCheckIn(courtId, level, notify = false) {
  try {
    const { data, error } = await supabase
      .from('check_ins')
      .insert({ court_id: courtId, level, notify })
      .select('id, court_id, level, created_at')
      .single();
...
// (switching replaces it), so taps can't inflate the count. `notify` (Supabase
// only) opts into pushing this vote to the signed-in voter's friends.
export async function checkIn(courtId, level, notify = false) {
  if (!LEVELS.includes(level)) throw new Error(`bad level: ${level}`);
  return isShared ? supaCheckIn(courtId, level, notify) : localCheckIn(courtId, level);
}

// Helpers for the occupancy snapshot attached to courts as `reserved`. Two
// sources feed it through one contract: SF's rec.us bookings
// (data/reservations.js) and NYC's Parks permits + online tennis reservations
// (data/cities/nyc/reservations.js). Slots are keyed "YYYY-MM-DD HH:MM" in the
// COURT'S OWN local time on a 30-min grid → how many courts are taken AT THAT
// SLOT. (Keys are wall-clock, and slotKeyOf below reads device-local fields, so
// a reading is correct for someone in the same metro as the court — true of
// both cities today.)
//
...
function readSlot(res, key, dir) {
  ...
  let raw = res.slots[key];
  // NYC permits ship SPARSE: a court+sport only carries the times somebody holds
  // it. Inside the swept `window` a missing key means nobody has it — genuinely
  // zero, not unknown — so read it as 0 rather than "no data". Outside the
  // window (and for every dense source) a missing key really is no data.
  if (raw == null) {
    if (!res.window || key < res.window[0] || key > res.window[1]) return null;
    raw = 0;
  }
  ...
  // Every court for this sport, per the SFRP directory — falls back to the source's
  // own count when a court has no directory entry (then walk-up courts stay
  // invisible). `res.total` is the source publishing that fuller count itself: NYC
  // tennis knows its reservation AND walk-on courts, so 2 booked of 2 reservation
  // courts at Riverside is 2 of 12, not "fully booked" with 10 walk-on courts free.
  const playable = Math.max(dir?.total ?? 0, res.total ?? 0, res.courts ?? 0) || null;
...
// Has this snapshot run out? Slots are keyed to ABSOLUTE dates on a rolling
// window, so a payload has a shelf life: once its horizon is behind us, every
// reading it can produce is null and the court renders as though it simply has
// no occupancy data. That is not what happened — we had data and it ran out, and
// "we no longer know" must not read as "nothing is booked". Callers use this to
// say so instead of silently falling through to the generic reservable line.
//
// Evaluated against NOW, never the viewed time: a dead snapshot is dead for
// every time you might look at, including future ones inside its old window.
export function snapshotExpired(res, now = new Date()) {
  if (!res || !res.slots) return false;
  // NYC permits ship sparse with an explicit swept `window`, whose end is the
  // real horizon — the last permit can fall days short of it. Dense sources
  // (rec.us) carry no window, so their newest key is the horizon.
  let horizon = res.window ? res.window[1] : '';
  if (!horizon) for (const k in res.slots) if (k > horizon) horizon = k;
  return !!horizon && horizon < slotKeyOf(now);
}

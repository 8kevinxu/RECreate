...
// Course term as a RELATIVE label for list cards, where the full range is both
// longer and half noise: a term that hasn't started answers "starts 9/12", one
// already running answers "through 8/22", and a single-date activity is just its
// date. Same hand-parsed M/D convention as formatDateRange (never new Date(iso),
// which lands on UTC midnight and reads a day early west of Greenwich). The year
// is deliberately omitted — the catalog only carries current and upcoming terms,
// so a month/day that has already passed can only mean next year, and the full
// dated range is one tap away in ClassDetail. Returns null with no start date
// (48 SF classes have none) so callers render nothing rather than a stray dot.
export function formatTermLabel(start, end) {
  if (!start) return null;
  const md = (iso) => {
    const [, m, d] = iso.split('-').map(Number);
    return `${m}/${d}`;
  };
  if (!end || end === start) return md(start); // one-day activity
  const n = new Date();
  const today = `${n.getFullYear()}-${String(n.getMonth() + 1).padStart(2, '0')}-${String(
    n.getDate()
  ).padStart(2, '0')}`;
  if (start > today) return tg('cls.startsOn', { d: md(start) });
  if (end >= today) return tg('cls.through', { d: md(end) });
  return formatDateRange(start, end); // wholly past — shouldn't ship, but don't hide it
}

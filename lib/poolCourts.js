...
// PUBLIC-SWIM sessions (lap/family/senior/youth/water-exercise/parent-tot — the "just
// show up and swim" ones), collected into a dropins.swimming week; lessons,
// camps, rentals and school groups are excluded from drop-in but still shown in the card's full
...
// Sessions anyone can drop in and swim (vs lessons/camps/rentals/school groups).
const PUBLIC_SWIM = new Set(['lap', 'family', 'senior', 'youth', 'exercise', 'parent_child']);
...
function toCourt(pool, city) {
  ...
  return {
    ...
    // Dated closures from the posters (holidays, maintenance, staff training).
    // lib/hours.js carves them out of the weekly blocks, so open-now, markers
    // and "Next:" labels respect them; PoolDetail says why.

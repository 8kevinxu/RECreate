...
// Shared token for the service's own auth check. Optional — a service running
// without ASSISTANT_TOKEN set ignores it, which is the localhost default.
//
// This is NOT a secret and must never be treated as one. Being an EXPO_PUBLIC_
// var it is inlined into the JS bundle, and a bundle can be read back out of any
// downloaded app; assume a determined person has it. What it buys is the removal
// of drive-by traffic — a port scanner, or someone who found the service URL —
// which is most of the volume an open endpoint actually attracts. The server's
// rate limits and daily budget are what hold when the token doesn't, so don't
// let the presence of this line imply the endpoint is protected by it.
const TOKEN = process.env.EXPO_PUBLIC_ASSISTANT_TOKEN || '';
...
class AssistantError extends Error {
  constructor(message, { retryable = true, detail = '', retryAfter = 0 } = {}) {
    super(message);
    this.name = 'AssistantError';
    this.retryable = retryable;
    this.detail = detail;
    this.retryAfter = retryAfter; // seconds; only set on 'busy'
  }
}
...
async function post(path, body, timeoutMs = TIMEOUT_MS) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 429) {
      // Asking too fast, or the service has spent its daily budget for everyone.
      // Retryable in the sense that waiting fixes it — the service sends a
      // Retry-After saying how long, which the UI surfaces as a wait hint rather
      // than as a countdown nobody watches.
      const seconds = Number(response.headers?.get?.('retry-after')) || 0;
      throw new AssistantError('busy', { detail, retryAfter: seconds });
    }
    if (response.status === 401 || response.status === 403) {
      // The build's token doesn't match the service's. A user retrying can't fix
      // that, so don't offer them the button.
      throw new AssistantError('unauthorized', { retryable: false, detail });
    }
...
export async function ask(history, { city, location, context, debug = false } = {}) {
  if (!enabled) throw new AssistantError('disabled', { retryable: false });

  // App.js stores a fix as { lat, lng }; Expo's own position objects use
  // { latitude, longitude }. Reading only the latter meant the app never sent
  // coordinates at all — every "near me" answer was computed without them, and
  // silently, because a missing origin just drops distance sorting. Accept both.
  ...
  const body = {
    ...
    state: {
      ...
      // What the user is looking at. Pointers, not facts: `court_id` tells the
      // service which court to look up, and it still reads every hour and price
      // from its own data.
      ...(context?.screen && { screen: context.screen }),
      ...(context?.courtId && { court_id: context.courtId }),
      ...(context?.courtName && { court_name: context.courtName }),

...
function surfaceFor() {
  ...
  if (/Android/i.test(ua)) return null; // no Android build to send them to
  ...
  if (iOS) {
    // Safari already renders Apple's smart banner from the meta tag. Every
    // other iOS browser (Chrome, Firefox, Edge, Opera) renders nothing, which
    // is exactly the gap the bar exists to cover.
    const otherBrowser = /CriOS|FxiOS|EdgiOS|OPiOS|GSA/.test(ua);
    return otherBrowser ? 'bar' : null;
  }

...
const READS_KEY = 'recreate:chatReads';
const DELETED_KEY = 'recreate:chatDeleted';
let channelSeq = 0;
...
export async function loadThreads(courtsById = {}) {
  ...
  const [{ data: rp }, { data: sp }] = await Promise.all([
    supabase.from('rec_run_participants').select('run_id').eq('user_id', me),
    supabase.from('rec_signal_participants').select('signal_id').eq('user_id', me),
  ]);
  ...
  const [runsRes, sigsRes, msgsRes] = await Promise.all([
    runIds.length
      ? supabase.from('rec_runs').select('id, court_id, starts_at, sport, status').in('id', runIds)
  ...
  // Hide blocked users: drop their messages (so group previews/unread ignore them)
  // and, below, remove direct threads with someone you've blocked.
  const blocked = await getBlockedIds();
  const messages = (msgsRes.data || []).filter((m) => !blocked.has(m.user_id));
  const reads = await getReads();
  ...
  const list = [...threads.values()].filter(
    (t) => !(t.kind === 'direct' && blocked.has(t.otherId))
  );
...
export async function sendMessage(thread, body) {
  if (!supabase) return { error: new Error(tg('err.chatNotConfigured')) };
  const me = await currentUserId();
  if (!me) return { error: new Error(tg('err.signInChat')) };

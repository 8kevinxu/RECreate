...
// Web uses the browser's own dialogs: react-native-web's Alert.alert is an empty
// function, so on web the button did nothing at all and no report was ever filed.
export function confirmReportData(refId) {
  const send = async () => {
    const { error } = await reportContent({ kind: 'data', refId });
    const msg = error ? error.message || tg('mod.fail') : tg('report.thanks');
    if (Platform.OS === 'web') window.alert(msg);
    else Alert.alert(msg);
  };
  if (Platform.OS === 'web') {
    if (window.confirm(`${tg('report.dataTitle')}\n\n${tg('report.dataBody')}`)) send();
    return;
  }
  Alert.alert(tg('report.dataTitle'), tg('report.dataBody'), [
    { text: tg('cancel'), style: 'cancel' },
    { text: tg('mod.report'), onPress: send },
  ]);
}

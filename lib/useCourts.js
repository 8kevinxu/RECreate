...
// courts), folded into every source list.
const EXTRA_COURTS = [...MANUAL_COURTS, ...SANBRUNO_COURTS, ...OUTDOOR_COURTS];

// Attach the rec.us "% booked" snapshot (court id -> { sport: { pct, courts } })
// onto matching courts as `reserved`, for the booked badge on the detail card.
function withReservations(list) {
  if (!Array.isArray(list)) return list;
  return list.map((c) => {
    const r = c && RESERVATIONS[c.id];
    return r ? { ...c, reserved: r } : c;
  });
}

...
export function buildRecommendations({
  ...
  now = new Date(),
  max = 10,
  // Classes are SF-only (bundled ActiveNet catalog) — a courts-only city passes
  // false so its users aren't recommended classes in another metro.
  includeClasses = true,
} = {}) {
  ...
  const classRecs = (includeClasses ? CLASSES : []).filter((cl) => cl.unlimited || cl.spots > 0)

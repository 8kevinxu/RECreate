...
//
// Returns, for one rec.us location, { at, bySport: { tennis|pickleball: {v, pct,
// courts, slots, blk?, overlapSport?, open?, released?, windows?, url} } } shaped
// exactly like a data/reservations.js per-sport entry so the app can overlay it
// directly, or null on failure. The slot math mirrors scripts/build-reservations.js
...
// The /schedule endpoint sits behind a WAF that 403s the short header set above.
const PAGE_HEADERS = {
...
// rec.us's own per-court day schedule — the only source that separates a real booking
// from a court blocked by a booking on the court it physically overlaps (see the header
// of scripts/build-reservations.js). One request covers the whole window.
...
async function fetchSchedule(locationId, win) {
  const url = `${API}/v1/locations/${locationId}/schedule?startDate=${win[0].date}&endDate=${win[win.length - 1].date}`;
...
// The other sport sharing this court's physical space, if any.
...
// { key, free, released, state } (see build-reservations.js courtSlots).
...
function locationBySport(location, win, sportMap, nowKey, now, url, sched) {
  const agg = {};
  ...
  for (const court of location.courts || []) {
    ...
    for (const sport of sports) {
      ...
      a.courts += 1;
      a.windows.add(hours);
      a.overlapSport ||= overlapSportOf(court, location, sportMap, sport);
      for (const { key, free, released, state } of slots) {
        const s = (a.slots[key] ||= { open: 0, rel: 0, booked: 0, res: 0, blk: 0 });
        s.open += 1;
  ...
  for (const [sport, a] of Object.entries(agg)) {
    const slots = {}, open = {}, released = {}, blk = {};
    for (const [key, s] of Object.entries(a.slots)) {
      if (s.rel === 0) continue; // nothing bookable then → no % to show
      slots[key] = s.res; // v2: reserved court counts (see lib/reservations.js)
      if (s.blk) blk[key] = s.blk;
    ...
    out[sport] = {
      ...
      ...(a.overlapSport && Object.keys(blk).length ? { overlapSport: a.overlapSport } : {}),
...
export async function fetchLiveReservations(locationId, force = false) {
  ...
  try {
    ...
    const now = sfDate();
    const win = windowDays(now);
    // Best-effort: without it we fall back to the availableSlots reading, which can't
    // tell a booking from an overlapping court's block.
    let sched = null;
    try {
      sched = await fetchSchedule(locationId, win);
    } catch {}

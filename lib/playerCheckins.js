...
export async function loadMyStats(userId) {
  ...
  // Sports the player has logged at their favorite court, most-played first (for the
  // "Favorite park: X (N check-ins for basketball, pickleball)" profile stat).
  const favoriteSports = favoriteCourtId
    ? Object.entries(perCourtSport)
        .filter(([k]) => k.slice(0, k.lastIndexOf('|')) === favoriteCourtId)
        .sort((a, b) => b[1] - a[1])

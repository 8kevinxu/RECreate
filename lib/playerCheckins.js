...
// Recent check-ins AT ONE COURT for one sport — what the court card counts as
// "N check-ins in the last hour" alongside the crowd's busyness reports.
//
// Deliberately NOT a community tally, and it can't be: RLS (migration 017) scopes
// these reads to your own rows plus accepted friends', because the alternative was
// letting any caller dump a location history keyed to user id. So a stranger's
// "I'm here" is invisible here by design, and the number is what YOU can see. A
// true community count needs an aggregate that returns a number rather than rows
// (a SECURITY DEFINER count RPC), which is a server change, not a client one.

...
// The check-ins at one court+sport the card can NAME. RLS (migration 017) scopes
// these reads to your own rows plus accepted friends', because the policy it
// replaced let any caller dump a location history keyed to user id — so a
// stranger's "I'm here" is deliberately invisible here. For how many there are in
// total, see countCourtVisits below; this answers "who", that answers "how many".
export async function loadCourtVisits(courtId, sport, windowMs = 60 * 60 * 1000) {
  if (!supabase || !courtId || !sport) return [];
  const me = await currentUserId();
  if (!me) return [];
...
// How many people checked in at one court+sport — EVERYONE's, not just the rows
// RLS lets you read. Backed by court_checkin_count (migration 025), a SECURITY
// DEFINER aggregate that returns a scalar and never rows, so the community number
// costs none of the row-level privacy 017 established.
//
// Returns null rather than 0 when it can't answer — an older database that hasn't
// run 025, a network failure — so callers fall back to the check-ins they can see
// instead of claiming an empty court.
export async function countCourtVisits(courtId, sport, windowMs = 60 * 60 * 1000) {
  if (!supabase || !courtId || !sport) return null;
  try {
    const { data, error } = await supabase.rpc('court_checkin_count', {
      p_court_id: courtId,
      p_sport: sport,
      p_minutes: Math.round(windowMs / 60000),
    });

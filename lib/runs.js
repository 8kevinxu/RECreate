// "Plan a game" — scheduled pickup games at a court (the hoop_runs table). A
...
export function subscribeRuns(onChange) {
  ...
  try {
    const channel = supabase
      .channel(`hoop_runs_${++channelSeq}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'hoop_runs' },
        onChange
      )
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'hoop_run_participants' },
        onChange
      )
      .subscribe();
    return () => {

...
// Fires a non-fatal test event to verify the Sentry pipeline end-to-end (used by
// a __DEV__-only button in Settings). Returns false when reporting is disabled.
export function captureTestError() {
  if (!crashReportingEnabled) return false;
  Sentry.captureException(new Error('RECreate Sentry test — ' + new Date().toISOString()));
  return true;
}

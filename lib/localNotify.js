...
// we schedule ONE on-device reminder for the day — the soonest still-upcoming
// interest-matched thing (an open-gym game for a favorite sport, or a rec class with
// openings in a favorite category), ~30 min before it starts — so you get a nudge
// even if the app is closed. Once it's scheduled, later foregrounds no-op for the
// rest of the day (deduped in AsyncStorage). Server push (for events while the app
// has never been opened) is a later addition; this is the MVP.
...
// One reminder a day, and that cap is doing more work than it looks like. Reminders
// are scheduled purely off each item's own start time, with no spacing between them,
// and the candidate list is sorted soonest-first — so the first N are the N earliest
// things today and tend to share a start time (six gyms that all open at 9 AM meant
// six notifications at 8:30). The near-term clamp below collapses them further: open
// the app at 8:50 and everything starting by 9:20 fires at 8:51, together. Raising
// this again means teaching the scheduler to space or coalesce first.
const MAX_PER_DAY = 1;
...
export async function syncInterestNotifications({
  ...
  try {
    ...
    // Couldn't read the ledger, so we don't know what's already pending — clear it
    // rather than schedule a second copy on top of it. This is the day's only
    // reminder, so a duplicate here is precisely the pile-up we're avoiding.
    try {
  ...
  for (const r of recs) {
    ...
    if (r.kind === 'sport') {
      ...
      startMin = r.startMin;
      // A notification title is plain text, so pickleball's drawn ball can't
      // appear here — omit its placeholder emoji rather than show a green dot.
      const glyph = r.sport === 'pickleball' ? '' : `${sportMeta(r.sport).emoji} `;
      title = `${glyph}${tg('sport.' + r.sport)} · ${r.courtName}`;
